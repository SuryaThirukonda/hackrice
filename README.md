...
A phone can play boxing alongside the keyboard: swing to punch with damage scaled by swing speed,
`A` to hold the guard, `B` to duck, D-pad left/right to slip and up/down to step in and out. Both
inputs are merged every frame, so either can be used at any time. Bowling and golf are keyboard-only
so far; the transport and the relay already carry their sport stamp.
