...
export default function Join() {
  ...
  return <main className="join-club">
    <header><span>✦ SPORTS CLUB</span><a href="/host">Host tools</a></header>
    ...
    <section className="join-card"><div><span className="club-tag">YOU'RE UP</span><h2>Controller 1</h2><p>Scan with your phone camera.<br/>Enable motion, connect, then calibrate.</p><a className="club-cta" href="/controller?player=1">Open controller →</a></div><div>{base ? <QR value={base + '/controller?player=1'} size={208} label="Scan to play" /> : <div className="qr-empty">Your join QR<br/>will appear here ↓</div>}</div></section>
    <div className="club-opponent"><span>🎮</span><div><b>Play a friend or Club Bot</b><p>Choose Human vs Human or Human vs AI in Godot.</p></div></div>
    <section className="join-card"><div><span className="club-tag">BRING A FRIEND</span><h2>Controller 2</h2><p>Scan on a second phone for two-human play.</p><a className="club-cta" href="/controller?player=2">Open Controller 2 →</a></div>{base && <QR value={base + '/controller?player=2'} size={208} label="Controller 2 · scan to play" />}</section>
    <details open={!base} className="club-settings"><summary>Phone join link & testing</summary><label>Paste the HTTPS tunnel address<input type="url" value={origin} onChange={e => setOrigin(e.target.value)} placeholder="https://your-tunnel.trycloudflare.com" /></label>{!base && <p>Phones need your HTTPS tunnel address for motion access. A localhost QR would point at the phone itself.</p>}<label><input type="checkbox" checked={testing} onChange={e => setTesting(e.target.checked)} /> Show extra Controller 2 test QR</label>{testing && base && <QR value={base + '/controller?player=2'} size={180} label="Controller 2 · testing" />}<a href="/controller?player=1&fake=1&debug=1">Desktop motion diagnostics</a></details>
    <footer>Choose your sport in Godot · B boxing · G mini golf · N bowling · F3 diagnostics</footer>
  </main>
}
