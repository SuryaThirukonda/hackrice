...
export function KeyboardPlayer() {
  ...
  const claim = () => {
    if (!openSeat?.join_url || sock.current) return
    const tok = openSeat.join_url.split('tok=')[1]
    const s = new ArenaSocket({ role: 'remote', token: tok, nickname: 'Keyboard', url: `${location.protocol === 'https:' ? 'wss' : 'ws'}://${location.host}/ws?role=remote&device=kb-${Math.random().toString(36).slice(2, 8)}` })
    s.on('session.welcome', (env) => { const d = env.d as { seat_id?: string | null; __open?: boolean }; if (d.__open) return; setClaimed(d.seat_id ?? null) })
    s.connect()
    sock.current = s
  ...
  useEffect(() => {
    const onEnter = (e: KeyboardEvent) => { if (e.key === 'Enter' && !sock.current) claim() }
  ...
  if (!claimed) return openSeat ? <button className="kb-claim plank" onClick={claim}><span className="art">⌨️</span><span className="name">Play with keyboard</span><span /><span className="sub">or press Enter</span></button> : null
  const sport = match?.sport ?? null
  const list = sport ? CONTROLS[sport] : []
  return (
    <div className="kb-hud">
      <div className="kb-title">Keyboard · {claimed}</div>
      ...
      {sport === 'boxing' && kb && <div className="kb-meters"><span className={kb.guard ? 'on' : ''}>{kb.guard ? 'GUARD UP' : 'guard down'}</span><span>{kb.last ?? ''}</span></div>}
