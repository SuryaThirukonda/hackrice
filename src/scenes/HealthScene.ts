...
export class HealthScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.add.text(W / 2, H - 22, '←→ body weight  ·  R refresh  ·  C clear history (press twice)  ·  Esc back', { fontFamily: FONT, fontSize: '14px', color: HEX(P.ink), fontStyle: '900', backgroundColor: HEX(P.paper), padding: { x: 12, y: 5 } }).setOrigin(0.5).setDepth(11)
    void this.refresh()
  }
  ...
  private render(s: HealthSummary): void {
    ...
    const t = s.today
    big(20, 56, `${Math.round(t.activeSeconds / 60)}`, 'active minutes', P.teal)
    ...
    SPORTS.forEach((sp, i) => {
      const b = s.bySport[sp], y = 330 + i * 40
      text(20, y, sp.toUpperCase(), 15, SPORT_COLOR[sp], DISPLAY)
      text(120, y + 1, `${b.sessions} session${b.sessions === 1 ? '' : 's'} · ${Math.round(b.activeSeconds / 60)} min · ~${Math.round(b.kcal)} kcal · ${b.swings} swings${b.romMean ? ` · ${Math.round(b.romMean)}° swing` : ''}`, 12, 0x5a4632)
    })

    // range of motion trend
    panel(410, 270, 440, 190)
    text(430, 286, 'RANGE OF MOTION', 22, P.ink, DISPLAY); text(660, 292, 'mean degrees per swing, by session', 12, 0x5a4632)
    if (s.romTrend.length < 2) text(430, 340, 'play two sessions with the phone to see a trend', 13, 0x5a4632)
    else {
    ...
    if (!L) text(890, 60, 'nothing recorded yet.\nplay a match with the phone\nconnected and it appears here.', 13, 0x5a4632)
      ...
      big(890, 100, `${L.activeMinutes}`, 'active min', P.teal); big(1060, 100, `~${Math.round(L.kcal)}`, 'kcal est.', P.orange)
    ...
    // weight + privacy
    panel(0, 480, 1250, 70, 0.3)
    text(20, 500, `BODY WEIGHT ${this.weightKg} kg`, 18, P.ink, DISPLAY); text(260, 506, '←→ to adjust · calories are estimated from movement intensity and this weight', 12, 0x5a4632)
    text(20, 528, 'Stored in a local database on this machine only. Not a medical device, not a diagnosis. Active minutes and swings are measured; calories and range of motion are estimates.', 11, 0x5a4632)
  }
}
