...
/** A punch and its recoil are over well inside this. Longer stillness means the player has reset. */
const FORWARD_RESET_MS = 300
...
class GestureDetector {
  ...
  // Retain the last punch direction for recoil rejection; allow a new axis.
  private opponentDirection: Vector3 | null = null
  private retracting = false
  private neutralSince: number | null = null
  private learningForward = false
  private swingQuietSince: number | null = null
  /** How long the phone has been at rest since the last punch. Once that reaches FORWARD_RESET_MS the
   *  learned forward axis is dropped, so the next punch may go in any direction. */
  ...
  reset(clearForward = false): void {
    ...
    this.learningForward = false
    this.swingQuietSince = null
  ...
  push(sample: ProcessedMotion): DetectedGesture | null {
    ...
    if (this.sport === 'boxing' && this.opponentDirection && this.state !== 'tracking') {
      // The forward axis exists only to reject the recoil of the punch just thrown. A phone that has
      // come to rest is no longer recoiling, so the axis is dropped and the next punch, in whatever
      // direction it is thrown, is a punch. Without this a player who changed grip or punched back
      // toward themselves was silently ignored as "retraction".
      if (magnitude(profiledAcceleration) <= config.releaseAcceleration) {
        this.stillSince ??= sample.t
        if (sample.t - this.stillSince >= FORWARD_RESET_MS) {
          this.opponentDirection = null
          this.retracting = false
          this.neutralSince = null
          this.stillSince = null
        }
    ...
    if (this.sport === 'boxing' && this.opponentDirection
      && this.state !== 'tracking' && !this.retracting
      && sample.t >= this.cooldownUntil
      && magnitude(profiledAcceleration) >= config.startAcceleration
      && Math.abs(dot(normalize(profiledAcceleration), this.opponentDirection)) < 0.65) {
      // Changing from an upright lift to a forward punching grip must not
      // leave the controller locked to its original axis. Opposite motion
      // along the same axis is still recoil, not a new direction.
      this.opponentDirection = null
    }
    ...
    if (this.state === 'idle') {
      ...
      this.learningForward = this.sport === 'boxing' && !this.opponentDirection
    ...
    // Establish forward from the strongest part of the first outward stroke,
    // not the first threshold sample (often a sideways wrist movement).
    // A reversal ends learning so recoil can never become the reference.
    if (this.learningForward) {
      if (dot(profiledAcceleration, active.accelerationDirection) < 0) {
        this.opponentDirection = [...active.accelerationDirection]
        this.learningForward = false
      } else if (magnitude(profiledAcceleration) > active.peakAcceleration) {
        active.accelerationDirection = normalize(profiledAcceleration)
      }
    }

    const positiveAcceleration = this.sport === 'boxing'
      ? positiveProjection(profiledAcceleration, active.accelerationDirection)
      : magnitude(profiledAcceleration)
    const positiveRotation = this.sport === 'boxing'
      ? (positiveAcceleration > config.releaseAcceleration ? sample.rotationMagnitude : 0)
    ...
    // Swings retain peaks across both sides of the arc, including the return.
    // The controller publishes only the best completed result after capture.
    if (this.sport !== 'boxing') {
      if (positiveAcceleration <= config.releaseAcceleration && sample.rotationMagnitude < config.armResetRotation) {
        this.swingQuietSince ??= sample.t
      } else this.swingQuietSince = null
      const swingSettled = this.swingQuietSince !== null && sample.t - this.swingQuietSince >= 80
      if (!swingSettled && duration < config.maxDurationMs) return null
    } else if (!settled && duration < config.maxDurationMs) return null
    ...
    // A gesture that crossed the start threshold must never arrive at the receiver as
    // a zero-power punch. Preserve the curve while giving real detections a
    // small non-zero floor.
    const power = Math.max(1, Math.round(100 * (
