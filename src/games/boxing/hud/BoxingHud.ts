...
export class BoxingHud {
  ...
  knockdownCount(n: number, who: 'a' | 'b'): void {
    ...
    if (this.countSub) {
      const pName = who === 'a' ? this.names[0] : this.names[1]
      const subTxt = this.is2p
        ? (n >= 8 ? `${pName} LAST CHANCE! GET UP!` : `${pName} IS DOWN! GET UP!`)
        : (who === 'a' ? (n >= 8 ? 'LAST CHANCE! GET UP NOW!' : 'GET UP! TAP KEYS / SHAKE!') : (n >= 8 ? 'ALMOST OUT!' : 'STAY DOWN!'))
      this.countSub.setText(subTxt)
      this.countSub.setColor(HEX(who === 'a' ? P.red : this.is2p ? P.red : P.ink))
    }
