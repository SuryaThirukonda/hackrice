...
export type BoxerModel = 'beginner' | 'intermediate' | 'pro' | 'boss'
...
export class OpponentRig {
  ...
  constructor(parent: Entity, color = P.red, trunks = P.blue, device?: import('playcanvas').GraphicsDevice, model: BoxerModel = 'beginner') {
    ...
    const skin = model === 'pro' ? 0xb9794d : SKIN
    const M = {
      skin: flatMat(skin), skinHi: flatMat(model === 'pro' ? 0xc68b5a : SKIN_HI), skinLo: flatMat(SKIN_LO),
    ...
    const shape = (parent: Entity, name: string, mat: import('playcanvas').StandardMaterial,
      ...
      return rounded && device ? facetedSphere(device, parent, name, mat, { ...options, bands: 3 }) : part(parent, name, 'box', mat, options)
    }
    const boss = model === 'boss', pro = model === 'pro', mid = model === 'intermediate'
    const width = boss ? 0.76 : pro ? 0.56 : mid ? 0.50 : 0.44
    ...
    this.torso = shape(this.body, boss ? 'hoodieTorso' : 'torso', cloth, [0,1.335,0], [width,0.46,boss ? 0.48 : 0.30], true)
