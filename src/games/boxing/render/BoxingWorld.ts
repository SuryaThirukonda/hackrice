...
export class BoxingWorld {
  ...
  constructor(engine: Engine3D, oppColor = P.red, spectator = false, is2p = false) {
    this.engine = engine
    ...
    if (is2p) {
      ...
      // P1 (Blue gloves, Red trunks) on left screen, facing P2
      // P2 (Red gloves, Blue trunks) on right screen, facing P1
      this.opp = new OpponentRig(this.root, P.red, P.blue, engine.app.graphicsDevice) // P2 3D body
      this.rigA = new OpponentRig(this.root, P.blue, P.red, engine.app.graphicsDevice) // P1 3D body
