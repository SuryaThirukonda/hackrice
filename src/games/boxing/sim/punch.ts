...
export function startPunch(f: Fighter, kind: PunchKind, dir: V2, out: Out): boolean {
  const fd = FRAME[kind]
  ...
  f.punch = kind; f.punchId++; f.resolved = false; f.thrown++; f.guard = false
...
export function resolvePunch(att: Fighter, def: Fighter, snap: DefSnap, dir: V2, out: Out): void {
  ...
  let dmg = fd.dmg * (1 + MOMENTUM_DMG * m) * fatigueDmg(att)
