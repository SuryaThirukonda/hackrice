...
/** Generates textures, waits for web fonts, shows a comic loading bar, then starts the title. */
export class BootScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    const ready = fonts ? Promise.all([fonts.load('32px "Lilita One"'), fonts.load('900 16px Nunito')]).catch(() => null) : Promise.resolve(null)
    this.tweens.add({ targets: prog, v: 1, duration: 700, ease: 'Sine.InOut', onUpdate: draw, onComplete: () => { void ready.then(() => { label.setText('READY'); this.time.delayedCall(150, () => this.scene.start('title')) }) } })
  }
}
