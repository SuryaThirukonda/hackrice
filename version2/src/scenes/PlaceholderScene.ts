...
export class PlaceholderScene extends Phaser.Scene {
  ...
  private title = ''; private sub = ''; private color = P.gold
  constructor() { super('placeholder') }
  init(d: { title?: string; sub?: string; color?: number }): void { this.title = d?.title ?? 'SOON'; this.sub = d?.sub ?? ''; this.color = d?.color ?? P.gold }
  create(): void {
    ensureTextures(this)
