...
| Bowling | `A`/`D` lane position, `Q`/`E` aim angle, `←`/`→` hook; the aim path sweeps on its own: tap `Space` to lock it, then hold `Space` and release for power, `Esc` pause |
| Golf | course chosen on the pre-fight screen; `W`/`S` club, `A`/`D` aim, `Space` three-press swing (start, power, accuracy), `Tab` top view, `Esc` pause |
...
- Lights: warm key directional with 1K PCF3 shadows, cool fill, warm rim spot; ACES tone mapping; an environment atlas generated from a painted gradient cubemap per world (`engine3d/env.ts`); linear fog per world.
- Materials (`engine3d/materials.ts`): 2-band toon lighting via shader chunks, hard specular dot and rim band; small procedural textures (`engine3d/textures.ts`: canvas weave, leather, wood, carpet, wall panels, grass, scuff decals); no emissive or bloom by design.
- Post (`engine3d/post.ts`): vignette and grading on medium; SSAO and 2K shadows only on high; low disables post and shadows. Set in Settings (3D QUALITY); medium is the default for laptops.
- Dressing: sagging ropes, turnbuckle pads, apron band, floor decals, ringside desk, stools, billboard crowd; bowling alley with neighbour lanes, monitors, lamps, seating; golf courses with themed props. Static ring furniture is batched into a handful of draw calls.
